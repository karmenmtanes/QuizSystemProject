...
export class TeacherUI {
  constructor(examService, resultService) {
    ...
    this.setupHTML();
    this.attachGlobalEventListeners(); // הוספת מאזין גלובלי לכל הכפתורים
  }

  setupHTML() {
    this.builderContainer.innerHTML = `
      <!-- אזור הצגת פרטי המבחן הפעיל -->
  ...
  // Event Delegation - מאזין אחד לכל הכפתורים
  attachGlobalEventListeners() {
    this.builderContainer.addEventListener("click", (e) => {
      ...
      // כפתורים בתוך רשימת המבחנים
      if (e.target.classList.contains("delete-btn")) this.deleteExam(e.target.dataset.id);
      if (e.target.classList.contains("edit-btn")) this.editExam(e.target.dataset.id);
      if (e.target.classList.contains("export-btn")) this.exportExam(this.examService.getExamById(e.target.dataset.id));
    });
  ...
  switchView(viewName) {
    ...
    if (viewName === 'manage') {
      ...
      this.renderChart(); // ציור מחדש של הגרף בעת מעבר למסך
    }
  }
  ...
  addQuestion() {
    ...
    const difficulty = document.getElementById("qDifficulty").value; // קריאת רמת הקושי
    ...
    // הוספת השאלה עם רמת הקושי
    const question = new Question(qText, [ans1, ans2, ans3, ans4], correctAns - 1, difficulty);
    this.currentExam.addQuestion(question);
    
    alert("השאלה נוספה בהצלחה!");
    document.querySelectorAll("#qText, #ans1, #ans2, #ans3, #ans4, #correctAns").forEach(i => i.value = "");
  }
  ...
  renderExamList() {
    ...
    list.querySelectorAll('.delete-btn').forEach(btn => btn.addEventListener('click', (e) => {
        if(confirm("האם למחוק?")) { this.examService.deleteExam(e.target.dataset.id); this.renderExamList(); }
    }));
    ...
    list.querySelectorAll('.export-btn').forEach(btn => btn.addEventListener('click', (e) => {
        const exam = this.examService.getExamById(e.target.dataset.id);
  ...
  renderStudentResults() {
    ...
    container.innerHTML = results.length === 0 ? "<p>אין עדיין תוצאות.</p>" : 
      results.map(r => `<p>סטודנט: ${r.studentUsername} | מבחן: ${r.examTitle} | ציון: ${r.percent}%</p>`).join('');
    
    // מעדכן גם את הגרף אם יש תוצאות
    if (document.getElementById("stats-view").style.display === 'block') {
       this.renderChart();
    }
  }

  // פונקציה לציור הגרף בעזרת Chart.js
  renderChart() {
    const results = this.resultService.getResults();
    ...
    // אם כבר יש גרף, נמחק אותו לפני שנצייר מחדש כדי למנוע כפילויות
    if (this.chartInstance) {
        this.chartInstance.destroy();
    }

    const labels = results.map(r => r.studentUsername + ' (' + r.examTitle + ')');
